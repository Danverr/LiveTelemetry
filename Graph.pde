...
class Graph extends GuiObject {
    ...
    int     xPos = 0, yPos = 0;    // location of the top left corner of the graph  
    int     Width, Height;         // Width and height of the graph
    
    color   GraphColor = color(0, 128);
    color   BackgroundColor = color(0, 128);
    color   StrokeColor = color(0, 128);
    ...
    // Selected font used for text 
    PFont   Font = RobotoMono_reg;
    ...
    void drawAxis() {        
        ...
        strokeWeight(1);        
        fill(TextColor);
        text(Title,xPos + Width / 2,yPos - 16);                    // Heading Title
        ...
        for (int x = 0; x <= xDiv; x++) {            
            ...
            // x-axis
            line( 
                float(x) / xDiv * Width + xPos - 3,yPos + Height,  //  x-axis Sub devisions 
        ...
        for (int y = 0; y <= yDiv; y++) {
            line(
                xPos - 3,float(y) / yDiv * Height + yPos, // ...
            ...
            if (RightAxis) {                
                ...
                line(
                    xPos + Width + 3,float(y) / yDiv * Height + yPos, // ...
                    xPos + Width + 7,float(y) / yDiv * Height + yPos  // Right Y axis sub devisions
                   );
                
                textAlign(LEFT); 
                
                String yAxisRight = str(yMinRight + float(y) / yDiv * (yMaxRight - yMinRight));
    ...
    void smoothLine(float[] x, float[] y) {        
        ...
        for (int i = 0; i < x.length; i++) {
            ...
            // none after last bit. So a straight line is drawn instead            
            if (i == 0 || i == x.length - 2) {
                line(
                    xPos + (x[i] - x[0]) / (x[x.length - 1] - x[0]) * Width,
                    yPos + Height - (y[i] / (yMax - yMin) * Height) + (yMin) / (yMax - yMin) * Height,
            ...
            // For the rest of the array a curve (spline curve)
            // can be created making the graph smooth.
            curveVertex(
                xPos + (x[i] - x[0]) / (x[x.length - 1] - x[0]) * Width,
                yPos + Height - (y[i] / (yMax - yMin) * Height) + (yMin) / (yMax - yMin) * Height
               );
